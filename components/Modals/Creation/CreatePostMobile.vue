            ...
            <div slot-scope="{ commands, isActive }" class="field has-addons">
              ...
              >
                ...
                <font-awesome-icon
                  v-else
                  icon="check"
                  class="is-white checkmark"
                />Live Post
              </BButton>
              <EmbedButton
                v-if="embedIsActive"
                :command="commands.iframe"
                @enterUrl="insertVideo"
              />
            </div>
          </editor-menu-bar>
  ...
  methods: {
    ...
    toggleLivePost() {
      if (this.postAsList.some(x => x.type === "project")) {
        ...
        this.$toast.open({
          duration: 6000,
          message:
            "Working on something? You must create a project in order to make a live post!",
          position: "is-top",
          type: "is-danger",
        });
      }
    ...
    async createPost() {
      ...
      if (this.livePost) {
        ...
        this.$router.push({
          path: `/beta/focus/${this.$store.state.user.username}`,
        });
